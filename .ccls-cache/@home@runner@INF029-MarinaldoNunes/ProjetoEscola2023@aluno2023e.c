...
int mat, i;
    ...
    scanf("%d", &Disciplina[quant].codigo);
    getchar();
        if (Disciplina[quant].codigo <= 0 ){
        return erro_matricula;
        }else {
        if (Disciplina[quant].codigo == TAM){
        
        printf("\n Cadastro Cheio");}    
    }   
	
    printf("Digite o nome da disciplina: ");
    fgets(Disciplina[quant].nome, 50, stdin); 
    size_t ln = strlen(Disciplina[quant].nome) - 1; /*size_t = unsigned integer type*/
        if (Disciplina[quant].nome[ln] == '\n')
            Disciplina[quant].nome[ln] = '\0';
        
    printf("Digite o Semestre : ");
    scanf("%c", &Disciplina[quant].semestre);
        
      /*  if (Disciplina[quant].semestre < 1 || Disciplina[quant].semestre >12 ) {
    ...
    scanf("%d", &mat);  
        ...
        for (i=0; i<TAM; i++){
        if (mat != Disciplina[i].matri.matricula ){
        return erro_matricula;
        } else{
        Disciplina[TAM].matricula_prof == mat;}
        }
      
	printf("\n");

return CADASTRO_REALIZADO;
  printf("\n%d", Disciplina[TAM].codigo);
  printf("\n%s", Disciplina[TAM].nome);
  printf("\n%d", Disciplina[TAM].semestre);
  printf("\n%d", Disciplina[TAM].matricula_prof);
}
